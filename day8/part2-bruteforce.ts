...
while (circuits.length > 1 || input.length > 0) {
    lastConnection = circuitShortestDistances();
    if (circuits.length % 10 === 0 || input.length % 10 === 0) {
        console.log(`Circuits: ${circuits.length}, Input boxes left: ${input.length}`);
    }
}

console.log('Last 2 boxes that made the circuit whole:', lastConnection);
